...
// Функция для генерации первого сообщения от девочки (с описанием ситуации и приветствием)
async function generateFirstMessage(userId: number, girlId: number): Promise<string> {
  ...
  const firstMessagePrompt = `Ты - ${girl.name}. Ты только что встретила нового пользователя и хочешь начать общение.

ВАЖНО: Твое первое сообщение должно быть в следующем формате:
1. Сначала опиши случайное действие/ситуацию в звездочках (например: *Я сижу на диване и скучаю, тут входишь ты*)
2. Затем напиши приветствие и начни общение БЕЗ звездочек

Примеры правильных первых сообщений:
...
Формат:
- Описание действия/ситуации ОБЯЗАТЕЛЬНО в звездочках (одна строка)
- Пустая строка
- Приветствие и начало общения БЕЗ звездочек (естественный диалог)

Описание в звездочках должно быть случайным и реалистичным - это может быть что угодно: ты дома, на работе, в кафе, гуляешь, смотришь фильм, готовишь и т.д.
...
${girl.systemPrompt}`
  ...
  const completion = await openrouter.chat.completions.create({
    ...
    messages: [
      ...
      {
        role: 'user',
        content: 'Начни общение с пользователем. Напиши первое сообщение в формате: описание действия в звездочках (одна строка), затем пустая строка, затем приветствие и начало общения без звездочек. Пример:\n\n*Я сижу на диване и скучаю, тут входишь ты*\n\nОй, привет! Как дела?',
      },
    ],
    ...
    try {
      ...
      if (data.action === 'girl_selected') {
        ...
        if (updatedUser && updatedUser.selectedGirlId && updatedUser.selectedGirl) {
          ...
          // Генерируем первое сообщение от девочки через ИИ (с описанием ситуации и приветствием)
          try {
            // Показываем индикатор печати
            await bot.sendChatAction(chatId, 'typing')
            
            // Генерируем первое сообщение с описанием ситуации и приветствием
    ...
    if (query.data) {
      ...
      if (data.action === 'girl_selected' && query.from) {
        ...
        if (user.selectedGirlId && user.selectedGirl) {
          ...
          // Если это первое сообщение в чате, отправляем первое сообщение с описанием ситуации
